...
function fetch_inputs(index) {
    ...
    $('#add_to_credits').prop('checked', jsonObj[index].add_to_credits === 1);
    $('#remarks').val(jsonObj[index].remarks);
}
function viewData(index) {
    fetch_inputs(index);
    $('#largeModalLabel').text('View Request Leave Credits Adjustment');
    ...
    $('#add_to_credits').attr('disabled',true);
    $('#btnSave,#btnCancel').hide();
}
function addData(index) {
    $('#largeModalLabel').text('Add Request Leave Credits Adjustment');
    $('#save-message,#delete-message').remove();
    ...
    $('#leave_credits_entry_description,#number_of_hours,#add_to_credits,#remarks').val('');
    $('#request_id,#leave_credits_entry_id').val(0);
    $('#add_to_credits').prop('checked',false);
    $('#add_to_credits').removeAttr('disabled',true);
}
function approve(index) {
	$('#largeModalLabel').text('Approve Request Leave Credits Adjustment');
    $('#save-message,#delete-message').remove();
    ...
    $('#delete-message-yes').click(function(){
        ...
        setTimeout(function(){
            ...
            executeService(jsonObjExecuteService, function(response_from_service) {
                ...
                if(success > 0){
                    jsonObjExecuteService = {};
                    jsonObjExecuteService ["procedure_name_given"] = "approveLeaveCreditAdjustment";  //given json element do not remove
                    jsonObjExecuteService ["method_given"] = "POST"; //given json element do not remove
                    jsonObjExecuteService ["module_given"] = "EmployeeProfile-API"; //given json element do not remove
                    // here is to create jsonObject needed for procedure
...
function cancel(index) {
	$('#largeModalLabel').text('Cancel Request Leave Credits Adjustment');
    $('#save-message,#delete-message').remove();
    const parentDiv = document.getElementById("message-form");
	parentDiv.innerHTML = `<span id="delete-message" class="control-label mb-1">
    ...
    $('#delete-message-yes').click(function(){
        ...
        setTimeout(function(){
            jsonObjExecuteService = {};
            jsonObjExecuteService ["procedure_name_given"] = "cancelLeaveCreditAdjustment";  //given json element do not remove
            jsonObjExecuteService ["method_given"] = "POST"; //given json element do not remove
            jsonObjExecuteService ["module_given"] = "EmployeeProfile-API"; //given json element do not remove
            // here is to create jsonObject needed for procedure
            jsonObjExecuteService ["request_id"] = jsonObj[index].request_id;
            ...
            setTimeout(function() {
                if (unique_application_user_id == 0 && typeof unique_application_user_id === undefined) {
                    check_user_id();
                        ...
                        jsonObj = [];
                        jsonObjExecuteService = {};
                        jsonObjExecuteService ["procedure_name_given"] = "getLeaveCreditAdjustment";  //given json element do not remove
                        ...
                        executeService(jsonObjExecuteService, function(response_from_service) {
                            ...
                            $('#approve_leave_credits_adjustment').parent().append('<label class="no-available-data-approved control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">🔍 Notice: Please Search Before Viewing Table Values.</label>');
                            $('#request_leave_credits_adjustment').parent().append('<label class="no-available-data-request control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">🔍 Notice: Please Search Before Viewing Table Values.</label>');
                        ...
                        function appendRequestTable(){
                            ...
                            var table = $('#request_leave_credits_adjustment')[0];
                        ...
                        function appendApproveTable(){
                            ...
                            var tableApprove = $('#approve_leave_credits_adjustment')[0];
                            ...
                            if (data.length !== 0) {
                                for (let i = 0; i < data.length; i++) {
                                    ...
                                    if(requestIdArray.length !== 0 && data[i].approved !== 0){
                                        ...
                                        $('.no-available-data-approved').remove();
                                        $('#approve_leave_credits_adjustment').parent().append('<label class="no-available-data-approved control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">No available data</label>');
                                    }
                                }
                            }else{
                                $('.no-available-data-approved').remove();
