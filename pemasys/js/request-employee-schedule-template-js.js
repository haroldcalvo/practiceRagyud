$(document).ready(function(){
    jsonObj = [];
    $('#req_emp_sched_table,#appr_emp_sched_table').parent().append('<label id="forNoDataReq" class="no-available-data control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">🔍 Quick Note: Kindly search before checking table values. Thanks!</label>');
    $('#req_emp_sched_table,#appr_emp_sched_table').children('tbody').children('tr').remove();
    document.getElementById('searchname').addEventListener("keypress", function (e) {
      if (e.key == "Enter") {
        ...
        e.preventDefault();
        populateScheduleTemplate(e.target.value,0);
      }
    });
    var storedID = localStorage.getItem('lastID');
    if (storedID) {
      populateScheduleTemplate("",storedID); 
      $('.no-available-data').remove();
      localStorage.removeItem('lastID');
    }
    function populateScheduleTemplate(reference,last_id) {
      const jsonData = {
        procedure_name_given : 'getRequestEmpSchedule',
        method_given : 'GET',
      ...
      executeService(jsonData, function(response_from_service) {
          populateRequestEmpTemplate(response_from_service);
      });
    }
  
    function populateRequestEmpTemplate(response) {

        // REQUEST
        $('#req_emp_sched_table').parent().append('<label id="forNoDataReq" class="no-available-data control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">No available data</label>');
        const parentDivReq = document.getElementById("req_emp_sched_table");
        const tbodyReq = parentDivReq.querySelector('tbody');
        $("#req_emp_sched_table").children("tbody").children('tr').remove();
        if(approving_officer > 0){
          $('#appr_emp_sched_table').parent().append('<label id="forNoDataAppr" class="no-available-data control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">No available data</label>');
        }
        function tbody_tr_td() {
          ...
          if(approving_officer > 0){
            ...
            const parentDivAppr = document.getElementById("appr_emp_sched_table");
            const tbodyAppr = parentDivAppr.querySelector('tbody');
            $("#appr_emp_sched_table").children("tbody").children('tr').remove();
            ...
            for (const employeeId in latestApprovedRecords) {
              $('#forNoDataAppr').remove();
                ...
                newRow.innerHTML = `<td class="autoCaps">${record.employee_name}</td>
                                    <td class="autoCaps">${record.template_name}</td>`;
              ...
              if (response[i].approved === 0) {
                ...
                if(response[i].requested_by == unique_application_user_id){
                  ...
                  newRow.innerHTML = `<td class="autoCaps">${response[i].employee_name}</td>
                                      <td class="autoCaps">${response[i].template_name}</td>
                                      <td>
        ...
        if(jsonObj.length == 0) {
          for(var i = 0; i < response.length; i++) {
            if (typeof response[i].request_id !== "undefined" ) {
                ...
                item["template_name"] = response[i].template_name;
              ...
              $('.no-available-data').remove();
              $('#req_emp_sched_table,#appr_emp_sched_table').parent().append('<label class="no-available-data control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">No available data</label>');
            }
          }
        }else {
          for(var i = 0; i < response.length; i++) {
        ...
        var tr_length = $('#req_emp_sched_table,#appr_emp_sched_table').children('tbody').children('tr').length;
...
function addData() {
  resetModal();
  $('#largeModalLabel').html('Add Request Employee Schedule Template');
  $('#entry_id, #employee_id, #template_id').val(0);
  $('#employee-name, #template').prop('readonly', false);
...
function fetch_inputs(index) {
  ...
  $('#template_id').val(jsonObj[index].template_id);
  $('#employee-name').val(jsonObj[index].employee_name);
  $('#template').val(jsonObj[index].template_name);
}

// Function to view data
function viewData(index) {
  resetModal();
  $('#largeModalLabel').text('View Request Employee Schedule Template');
  fetch_inputs(index);
  $('#employee-name, #template').prop('readonly', true);
...
function viewDataApproved(index) {
  resetModal();
  $('#largeModalLabel').text('View Approved Employee Schedule Template');
  fetch_inputs(index);
  $('#employee-name, #template').prop('readonly', true);
  $('#btnSave, #btnCancel').hide();
}
...
function updateData(index) {
  resetModal();
  $('#largeModalLabel').text('Update Request Employee Schedule Template');
  fetch_inputs(index);
  $('#employee-name, #template').prop('readonly', false);
}
//DELETE DATA
function deleteData(index) {
  resetModal();
  $('#btnSave, #btnCancel').hide();
  $('#largeModalLabel').text('Delete Request Employee Schedule Template');
  fetch_inputs(index);
  $('#employee-name, #template').attr('disabled',true);
  ...
  parentDiv.innerHTML = `<span id="delete-message" class="control-label mb-1">
        ...
        const jsonData = {
            procedure_name_given : 'deleteRequestEmployeeTemplate',
            method_given : 'DELETE',
            module_given : 'EmployeeProfile-API',
...
function approve(index) {
  resetModal();
  $('#btnSave, #btnCancel').hide();
  $('#largeModalLabel').text('Approve Request Employee Schedule Template');
  fetch_inputs(index);
  $('#employee-name, #template').attr('disabled',true);
  const parentDiv = document.getElementById("message-form");
  parentDiv.innerHTML = `<span id="delete-message" class="control-label mb-1">
        ...
        $('#delete-message-yes').click(function(){
          ...
          const jsonData = {
            procedure_name_given : 'postJSONApproveEmpSchedTemplates',
            method_given : 'POST',
            module_given : 'EmployeeProfile-API',
            request_id : jsonObj[index].request_id,
            employee_id : jsonObj[index].employee_id,
            template_id : jsonObj[index].template_id,
...
$(document).ready(function(){
    function validationAttr() {
        $('#employee-name, #template').prop("disabled", true);
    }

    function validationRemoveAttr() {
        $('#employee-name, #template').prop("disabled", false);
    }
    $("#btnSave").click(function(){
        ...
        //VALDIATION
        $('#employee-name, #template').removeClass('is-invalid');
        if (!employee_id || !template_id || !employee || !template) {
          if (!employee_id) {
        ...
        const jsonData = {
            procedure_name_given : 'postJSONRequestEmpSchedTemplates',
            method_given : 'POST',
            module_given : 'EmployeeProfile-API',
