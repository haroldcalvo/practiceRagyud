$(document).ready(function(){
    jsonObj = [];
    $('#req_emp_sub_table,#appr_emp_sub_table').parent().append('<label id="forNoDataReq" class="no-available-data control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">🔍 Quick Note: Kindly search before checking table values. Thanks!</label>');
    $('#req_emp_sub_table,#appr_emp_sub_table').children('tbody').children('tr').remove();
    document.getElementById('searchname').addEventListener("keypress", function (e) {
      if (e.key == "Enter") {
          ...
          e.preventDefault();
          populateSubDepartment(e.target.value,0);
      }
    });
    var storedID = localStorage.getItem('lastID');
    if (storedID) {
      populateSubDepartment("",storedID); 
      $('.no-available-data').remove();
      localStorage.removeItem('lastID');
    }
    function populateSubDepartment(reference,last_id) {
      const jsonData = {
        procedure_name_given : 'getRequestEmpSubDepartment',
        method_given : 'GET',
      ...
      executeService(jsonData, function(response_from_service) {
          populateRequestEmpSubDepartment(response_from_service);
      });
    }
  
    function populateRequestEmpSubDepartment(response) {
        // REQUEST
        $('#req_emp_sub_table').parent().append('<label id="forNoDataReq" class="no-available-data control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">No available data</label>');
        const parentDivReq = document.getElementById("req_emp_sub_table");
        const tbodyReq = parentDivReq.querySelector('tbody');
        $("#req_emp_sub_table").children("tbody").children('tr').remove();
        if(approving_officer > 0){
          $('#appr_emp_sub_table').parent().append('<label id="forNoDataAppr" class="no-available-data control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">No available data</label>');
        }
        function tbody_tr_td() {
          if(approving_officer > 0){
            // APPROVED
            const parentDivAppr = document.getElementById("appr_emp_sub_table");
            const tbodyAppr = parentDivAppr.querySelector('tbody');
            $("#appr_emp_sub_table").children("tbody").children('tr').remove();
            ...
            for (const employeeId in latestApprovedRecords) {
              $('#forNoDataAppr').remove();
                ...
                newRow.innerHTML = `<td class="autoCaps">${record.employee_name}</td>
                                    <td class="autoCaps">${record.sub_name}</td>`;
              ...
              if (response[i].approved === 0) {
                ...
                if(response[i].requested_by == unique_application_user_id){
                  ...
                  newRow.innerHTML = `<td class="autoCaps">${response[i].employee_name}</td>
                                      <td class="autoCaps">${response[i].sub_name}</td>
                                      <td>
        ...
        if(jsonObj.length == 0) {
          for(var i = 0; i < response.length; i++) {
            if (typeof response[i].request_id !== "undefined" ) {
              ...
              $('.no-available-data').remove();
              $('#req_emp_sub_table,#appr_emp_sub_table').parent().append('<label class="no-available-data control-label mb-1" style="padding-bottom:15px;padding-top:15px;text-align:center;display: block;">No available data</label>');
            }
          }
        }else {
          for(var i = 0; i < response.length; i++) {
        ...
        var tr_length = $('#req_emp_unit_table,#appr_emp_unit_table').children('tbody').children('tr').length;
...
function addData() {
  resetModal();
  $('#largeModalLabel').html('Add Request Employee Sub-Department');
  $('#entry_id, #employee_id, #sub_id').val(0);
  $('#employee-name, #subDepartment').prop('readonly', false);
...
function fetch_inputs(index) {
  ...
  $('#sub_id').val(jsonObj[index].sub_id);
  $('#employee-name').val(jsonObj[index].employee_name);
  $('#subDepartment').val(jsonObj[index].sub_name);
}

// Function to view data
function viewData(index) {
  resetModal();
  $('#largeModalLabel').text('View Request Employee Sub-Department');
  fetch_inputs(index);
  $('#employee-name, #subDepartment').prop('readonly', true);
  $('#btnSave, #btnCancel').hide();
}
// Function to view approved data
function viewDataApproved(index) {
  resetModal();
  $('#largeModalLabel').text('View Approved Employee Sub-Department');
  fetch_inputs(index);
  $('#employee-name, #subDepartment').prop('readonly', true);
  $('#btnSave, #btnCancel').hide();
}
...
function updateData(index) {
  resetModal();
  $('#largeModalLabel').text('Update Request Employee Sub-Department');
  fetch_inputs(index);
  $('#employee-name, #subDepartment').prop('readonly', false);
}
...
function deleteData(index) {
  resetModal();
  $('#btnSave, #btnCancel').hide();
  $('#largeModalLabel').text('Delete Request Employee Sub-Department');
  fetch_inputs(index);
  $('#employee-name, #subDepartment').attr('disabled',true);
  ...
  parentDiv.innerHTML = `<span id="delete-message" class="control-label mb-1">
        ...
        const jsonData = {
            procedure_name_given : 'deleteRequestEmpSubDepartment',
            method_given : 'DELETE',
            module_given : 'EmployeeProfile-API',
...
function approve(index) {
  resetModal();
  $('#btnSave, #btnCancel').hide();
  $('#largeModalLabel').text('Approve Request Employee Sub-Department');
  fetch_inputs(index);
  $('#employee-name, #subDepartment').attr('disabled',true);
  const parentDiv = document.getElementById("message-form");
  parentDiv.innerHTML = `<span id="delete-message" class="control-label mb-1">
        ...
        $('#delete-message-yes').click(function(){
          ...
          const jsonData = {
            procedure_name_given : 'postJSONApproveEmpSubDepartment',
            method_given : 'POST',
            module_given : 'EmployeeProfile-API',
            request_id : jsonObj[index].request_id,
            employee_id : jsonObj[index].employee_id,
            sub_id : jsonObj[index].sub_id,
...
$(document).ready(function(){
    function validationAttr() {
        $('#employee-name, #subDepartment').prop("disabled", true);
    }

    function validationRemoveAttr() {
        $('#employee-name, #subDepartment').prop("disabled", false);
    }
    $("#btnSave").click(function(){
        ...
        //VALDIATION
        $('#employee-name, #subDepartment').removeClass('is-invalid');
        if (!employee_id || !sub_id || !employee || !subDepartment) {
          if (!employee_id) {
          ...
          if (!sub_id) {
            $('#subDepartment').addClass('is-invalid');
          }else{
            $('#subDepartment').addClass('is-valid');
          }
          if (!employee) {
            $('#employee-name').addClass('is-invalid');
          }else{
        ...
        const jsonData = {
            procedure_name_given : 'postJSONRequestEmpSubDep',
            method_given : 'POST',
            module_given : 'EmployeeProfile-API',
